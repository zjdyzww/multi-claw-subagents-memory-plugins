...
export class EventBus extends EventEmitter {
     ...
     * 订阅事件
     */
    subscribe(agentId, eventTypes, callback, filter) {
     ...
     * 发布记忆创建事件
     */
    publishMemoryCreated(agentId, repoType, docId, path) {
        this.publish({
            type: 'memory.created',
            agentId,
            repoType: repoType,
     ...
     * 发布记忆更新事件
     */
    publishMemoryUpdated(agentId, repoType, docId, path, changes) {
        this.publish({
            type: 'memory.updated',
            agentId,
            repoType: repoType,
     ...
     * 发布记忆删除事件
     */
    publishMemoryDeleted(agentId, repoType, docId, path) {
        this.publish({
            type: 'memory.deleted',
            agentId,
            repoType: repoType,
     ...
     * 发布同步完成事件
     */
    publishMemorySynced(agentId, repoType, result) {
        this.publish({
            type: 'memory.synced',
            agentId,
            repoType: repoType,
     ...
     * 发布冲突检测事件
     */
    publishConflictDetected(agentId, repoType, conflicts) {
        this.publish({
     ...
     * 获取事件日志
     */
    getEventLog(limit = 100, eventType) {
