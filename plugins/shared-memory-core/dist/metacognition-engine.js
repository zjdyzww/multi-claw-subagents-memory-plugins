...
export class MetacognitionEngine extends EventEmitter {
    ...
    assess(document) {
        ...
        // 综合评分
        const overall = Math.round((completenessScore + freshnessScore + consistencyScore + confidenceBalanceScore) / 4);
        const report = {
    ...
    // 评分函数
    // ============================================================
    scoreCompleteness(doc, issues, recs) {
        let score = 100;
    ...
    scoreFreshness(doc, issues, recs) {
        let score = 100;
    ...
    scoreConsistency(doc, issues, recs) {
        let score = 100;
        // Author consistency
    ...
    scoreConfidenceBalance(doc, issues, recs) {
        let score = 100;
        if (!doc.confidence) {
            score -= 25;
...
//# sourceMappingURL=metacognition-engine.js.map