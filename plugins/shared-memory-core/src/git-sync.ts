...
export class GitSyncManager extends EventEmitter {
   ...
   * 同步仓库（pull + push）
   */
  async syncRepo(repoType: string, options: GitOptions = {}): Promise<SyncResult> {
    ...
    try {
      ...
      if (status.conflicted.length > 0) {
        const conflicts: Conflict[] = status.conflicted.map(file => ({
          filePath: file,
          localSHA: '',
          remoteSHA: '',
          conflictType: 'modify-modify' as const,
          resolution: 'pending'
        }));
   ...
   * 获取仓库状态
   */
  async getRepoStatus(repoType: string): Promise<{
    ahead: number;
    behind: number;
    conflicted: string[];
    modified: string[];
   ...
   * 解决冲突
   */
  async resolveConflicts(repoType: string, conflicts: Array<{ file: string; resolution: 'local' | 'remote' | 'merged' }>): Promise<void> {
    const git = this.gits.get(repoType);
    if (!git) {
      throw new Error(`Repository ${repoType} not found`);
    }
