...
export class ResidualEngine extends EventEmitter {
  private queue: ResidualEntry[] = [];
  private cleanupHistory: CleanupRecord[] = [];
  private timer: ReturnType<typeof setInterval> | null = null;
  private checkIntervalMs: number;

  // 三层时间阈值
  private readonly LAYER1_MS = 24 * 60 * 60 * 1000;  // 24h
  private readonly LAYER2_MS = 7 * 24 * 60 * 60 * 1000; // 7d
  private readonly LAYER3_MS = 30 * 24 * 60 * 60 * 1000; // 30d
  ...
  enqueue(fact: FactPoint): void {
    if (this.queue.find(e => e.fact.id === fact.id)) return;
   ...
   */
  private computeAgeWeight(ageMs: number, layer: number): number {
    switch (layer) {
      case 1: {
