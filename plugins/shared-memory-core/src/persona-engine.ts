...
import type { MemoryRepresentation, FactPoint, ConfidenceLevel } from './types.js';
...
interface PersonaDefinition {
  ...
  activationScore: number;  // 0-100, 当前激活度
  minThreshold: number;     // 激活阈值
}
...
export class PersonaEngine extends EventEmitter {
  ...
  private collaborationHistory: CollaborationResult[] = [];
  ...
  activateByKeywords(text: string): PersonaDefinition[] {
    ...
    const activated: PersonaDefinition[] = [];
   ...
   */
  async collaborate(input: MemoryRepresentation): Promise<CollaborationResult> {
    ...
    // 投票 → 最高票数的置信度当选
    const votes: Record<ConfidenceLevel, number> = {
      CONFIRMED: 0,
      LIKELY: 0,
      UNCERTAIN: 0,
    };
    ...
    // 确定最终置信度
    let consensusConfidence: ConfidenceLevel = 'UNCERTAIN';
    const voteEntries = Object.entries(votes) as [ConfidenceLevel, number][];
   ...
   * 获取协作历史
   */
  getHistory(limit = 10): CollaborationResult[] {
   ...
   * 单个专家对输入进行评估
   */
  private getExpertOpinion(persona: PersonaDefinition, input: MemoryRepresentation): ExpertOpinion {
  ...
  private architectEvaluate(text: string, input: MemoryRepresentation, suggestions: string[]): string {
    const parts: string[] = [];
  ...
  private architectConfidence(text: string): ConfidenceLevel {
    if (/L[1-4]|分层|金字塔|架构/.test(text)) return 'CONFIRMED';
    if (text.length > 100) return 'LIKELY';
    return 'UNCERTAIN';
  }

  private reviewerEvaluate(_text: string, input: MemoryRepresentation, suggestions: string[]): string {
    const parts: string[] = [];
  ...
  private reviewerConfidence(input: MemoryRepresentation): ConfidenceLevel {
    const uncertain = input.facts.filter(f => f.confidence === 'UNCERTAIN').length;
    const total = input.facts.length || 1;
    if (uncertain / total > 0.5) return 'UNCERTAIN';
    if (uncertain / total > 0.2) return 'LIKELY';
    return 'CONFIRMED';
  }

  private criticEvaluate(text: string, input: MemoryRepresentation, suggestions: string[]): string {
    const parts: string[] = [];
  ...
  private criticConfidence(input: MemoryRepresentation): ConfidenceLevel {
    ...
    if (hasContradictions) return 'UNCERTAIN';
    if (verified / total < 0.3) return 'LIKELY';
    return 'CONFIRMED';
  }

  private integratorEvaluate(_text: string, input: MemoryRepresentation, suggestions: string[]): string {
    const parts: string[] = [];
  ...
  private integratorConfidence(input: MemoryRepresentation): ConfidenceLevel {
    const levels = [...new Set(input.facts.map(f => f.confidence))];
    const confirmed = input.facts.filter(f => f.confidence === 'CONFIRMED').length;
    const total = input.facts.length || 1;

    if (levels.length === 1 && levels[0] === 'CONFIRMED') return 'CONFIRMED';
    if (confirmed / total >= 0.5) return 'LIKELY';
    return 'UNCERTAIN';
  }
}
